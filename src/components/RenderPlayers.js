import { fetchAllPlayers, fetchSinglePlayer, addNewPlayer, removeNewPlayer } from './FetchPlayers'
...
export const RenderAllPlayers = (playerList) => {
    ...
    for (let i = 0; i < detailButtons.length; i++) {
        ...
        button.addEventListener('click', async () => {
            /*YOUR CODE HERE*/
            const pupId = await fetchSinglePlayer(button.dataset.id);
            renderSinglePlayer(pupId);
        });
    }
...
export const RenderSinglePlayer = (playerObj) => {
  ...
  `;
    ...
    let backToAllButton = document.getElementById('see-all');
